...
func writeDepLock(path string, dl *DepLock) error {
	...
	if err != nil {
		return fmt.Errorf("umbra: marshal %s: %w", LockName, err)
	}
	if err := os.WriteFile(path, append(b, '\n'), 0o644); err != nil { //nolint:gosec // committed source-of-truth lock, not a secret
		return fmt.Errorf("umbra: write %s: %w", path, err)
	}
	return nil
}
...
func readDepLock(path string) (*DepLock, error) {
	...
	if err := json.Unmarshal(b, &dl); err != nil {
		return nil, fmt.Errorf("umbra: parse %s: %w", path, err)
	}
	if dl.Version != depLockVersion {
		return nil, fmt.Errorf("umbra: %s schema version %d unsupported (want %d); re-run 'umbra lock'", LockName, dl.Version, depLockVersion)
	}
	return &dl, nil
}
...
func resolveDepLock(dir, ref, replace string) (*DepLock, error) {
	...
	if replace != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("umbra: resolve umbra replace path: %w", err)
		}
		fmt.Fprintf(&b, "\nreplace %s => %s\n", cliGuardModule, abs)
	}
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte(b.String()), 0o600); err != nil {
		return nil, fmt.Errorf("umbra: seed build go.mod: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("umbra: read resolved go.mod: %w", err)
	}
	goSum, err := os.ReadFile(filepath.Join(dir, "go.sum")) //nolint:gosec // path under driver-owned temp dir
	if err != nil {
		return nil, fmt.Errorf("umbra: read resolved go.sum: %w", err)
	}
	cg := cgVersion
	if replace != "" {
...
func writeModuleFiles(dir string, dl *DepLock) error {
	...
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte(mod), 0o600); err != nil {
		return fmt.Errorf("umbra: write build go.mod: %w", err)
	}
	...
	if err := os.WriteFile(filepath.Join(dir, "go.sum"), []byte(sum), 0o600); err != nil {
		return fmt.Errorf("umbra: write build go.sum: %w", err)
	}
	return nil
}
