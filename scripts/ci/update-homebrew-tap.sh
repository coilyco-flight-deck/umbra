#!/usr/bin/env bash
# Bump the specgen formula in the tap. Absent token is a skip rather than a
...
cp dist/specgen.rb tap/Formula/specgen.rb
cd tap
git add Formula/specgen.rb
...
git commit -m "chore(specgen): bump formula to ${TAG} [skip ci]"
git push \
