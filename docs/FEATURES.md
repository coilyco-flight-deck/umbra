...
- **opcore** - The frozen inline grammar: typed query, body projection, GraphQL and SQL grants, JMESPath postconditions, MCP proxy grants, and a named client. A body object may be `keyed` (caller keys sharing one `entry` shape) or a `variant` (a discriminated union), lowering to `additionalProperties`/`oneOf`. See [opcore-inline.md](opcore-inline.md), [opcore-body.md](opcore-body.md), and [opcore-body-variants.md](opcore-body-variants.md).
