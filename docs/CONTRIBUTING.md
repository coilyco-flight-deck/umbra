...
3. **Run the dev verbs before pushing.** Local dev routes straight through `just` (umbra is the framework itself, not a guarded consumer):
...
Pull requests authored or substantially edited by an LLM-driven agent are welcome. See [AGENTS.md](https://github.com/coilyco-flight-deck/umbra/blob/main/AGENTS.md) for the conventions a contributing agent should follow (issue-first, `Dangerously*` naming, dev-verb routing through `just`, etc).
