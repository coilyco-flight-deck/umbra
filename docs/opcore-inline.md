...
- **query / body** - flat names become string fields, and blocks add typed, bounded, aliased, or exclusive ones. **set** becomes `FixedBody`. See [body projection and pins](opcore-body.md).
- **fail-when** - a JMESPath predicate over a success response, and truthy fails the call. Inputs are `$name` variables.
